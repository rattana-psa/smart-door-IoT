...
void recordTime(int d, int m, int h, int mi){
  myFile = SD.open("time.txt", FILE_WRITE); // เปิดไฟล์ที่ชื่อ time.txt เพื่อเขียนข้อมูล โหมด FILE_WRITE
  ...
  // เปิดไฟล์เพื่ออ่าน
  myFile = SD.open("time.txt"); // สั่งให้เปิดไฟล์ชื่อ time.txt เพื่ออ่านข้อมูล
  if (myFile) {
